...
const ViewBook: React.FC = () => {
  ...
  const [priceFilter, setPriceFilter] = useState<RequestBody['operator']>('greater');
...
{viewOption !== 'all' && (
  <div className="flex flex-col gap-4 mb-4">
    {['stock', 'price'].includes(viewOption) && (
      <div className="flex gap-4">
        <label className="flex items-center gap-2">
          <input
            type="radio"
            name={`${viewOption}Filter`}
            value="less"
            checked={(viewOption === 'stock' ? stockFilter : priceFilter) === 'less'}
            onChange={() =>
              viewOption === 'stock'
                ? setStockFilter('less')
                : setPriceFilter('less')
        ...
        <label className="flex items-center gap-2">
          <input
            ...
            name={`${viewOption}Filter`}
            value="greater"
            checked={(viewOption === 'stock' ? stockFilter : priceFilter) === 'greater'}
            onChange={() =>
              viewOption === 'stock'
                ? setStockFilter('greater')
        ...
        <label className="flex items-center gap-2">
          <input
            ...
            name={`${viewOption}Filter`}
            value="equal"
            checked={(viewOption === 'stock' ? stockFilter : priceFilter) === 'equal'}
            onChange={() =>
              viewOption === 'stock'
                ? setStockFilter('equal')
                : setPriceFilter('equal')
            }
          />
    ...
    />
          ...
          <table className="table-auto w-full border-collapse border border-gray-300 text-accent-moonstone">
            ...
            <tbody>
              {books.map((book) => (
                <tr
                  key={book.book_id}
                  className={`cursor-pointer ${selectedBook?.book_id === book.book_id ? 'bg-accent-lightblue' : ''}`}
      ...
      {selectedBook && selectedBook.imageUrl && (
        <div className="mt-4 p-4 border border-gray-300 rounded-lg bg-white">
          <h4 className="text-lg font-bold text-accent-moonstone">Preview</h4>
          <img
            src={selectedBook.imageUrl}
            alt={`Preview of ${selectedBook.title}`}
            className="w-full max-w-sm h-auto rounded-lg mt-2"
          />
          <p className="mt-2 text-accent-raisin_black">{selectedBook.title}</p>
